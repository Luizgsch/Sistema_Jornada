...
import { mockContatosComunicacao, mockMensagensPorContato } from '@/data/mock/mockComunicacao';
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] -m-8 p-4 pt-0">
      <div className="mb-4 pt-4 px-4 shrink-0">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Comunicação e Relacionamento</h1>
        <p className="text-sm text-muted-foreground mt-1">Central de WhatsApp e E-mail integrada ao CRM do Candidato.</p>
      </div>

      {/* Main App Container */}
      <div className="flex-1 overflow-hidden bg-white border border-slate-200 shadow-sm rounded-2xl flex">
        ...
        {/* Center - Chat Area */}
        <div className="flex-1 flex flex-col min-w-0 bg-slate-50/50 bg-[url('https://th.bing.com/th/id/R.546ee5fb4ebbd7bdef7f3c4dbb718501?rik=z%2fy%2fH9MIFM4pBQ&pid=ImgRaw&r=0')] bg-blend-soft-light bg-opacity-40">
          
          {/* Chat Header */}
          <div className="h-16 px-6 border-b border-slate-200 bg-white/95 backdrop-blur-sm flex items-center justify-between shadow-sm shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-slate-800 leading-tight">{activeContact.nome}</h3>
            ...
            <div className="flex gap-2 text-slate-500">
              ...
              <button className="p-2 hover:bg-slate-100 rounded-full"><Search size={18} /></button>
                  ...
                  <div className={cn(
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className={cn(
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className={cn("text-xs", msg.remetente === 'rh' ? "text-white/70" : "text-slate-400")}>PDF Document</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {msg.tipo === 'convite' && (
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-3 p-3 bg-white/20 rounded-lg mt-1 border border-white/20">
          ...
          <div className="px-6 pb-2 pt-2 flex gap-2 overflow-x-auto no-scrollbar">
            <button onClick={() => handleQuickAction("Agendar Entrevista")} className="shrink-0 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-semibold text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-colors flex items-center gap-1.5 shadow-sm">
              <Calendar size={14} /> Agendar Entrevista
            </button>
            <button onClick={() => handleQuickAction("Pedir Documentos")} className="shrink-0 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-semibold text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-colors flex items-center gap-1.5 shadow-sm">
              <FileText size={14} /> Solicitar Documentos
            </button>
            <button onClick={() => handleQuickAction("Aprovar")} className="shrink-0 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-semibold text-emerald-600 hover:bg-emerald-500 hover:text-white transition-colors flex items-center gap-1.5 shadow-sm">
        ...
        <div className="w-72 border-l border-slate-200 bg-white flex flex-col shrink-0 overflow-y-auto custom-scrollbar">
          ...
          <div className="p-6 flex-1 space-y-6">
            ...
            <section>
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Documentos Anexos</h4>
              {activeContact.documentosEnviados.length === 0 ? (
                <p className="text-sm text-slate-500 italic">Nenhum documento.</p>
              ) : (
                <ul className="space-y-2">
                  {activeContact.documentosEnviados.map((doc, idx) => (
                    <li key={idx} className="flex items-center justify-between p-2 bg-slate-50 border border-slate-100 rounded-lg group hover:border-slate-300 transition-colors cursor-pointer">
                      ...
                      <ChevronRight size={14} className="text-slate-300 group-hover:text-primary shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </li>
                  ))}
                </ul>
              )}
