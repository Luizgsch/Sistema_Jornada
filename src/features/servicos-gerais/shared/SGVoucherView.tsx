...
import { mockVouchersNatal } from '@/data/mock/mockServicosGerais';
        ...
        <CardContent className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1 space-y-4 w-full max-w-md">
            ...
            <div className="p-4 rounded-xl bg-slate-900 text-white">
              <p className="text-xs text-slate-400 uppercase">Payload (simulado)</p>
              <p className="font-mono text-xs break-all mt-2 text-slate-200">{v.qrPayload}</p>
              <p className="text-xs mt-4 text-slate-400">Status: {v.emitido ? 'Emitido' : 'Pré-visualização'}</p>
            </div>
            <button
              type="button"
          ...
          <div className="w-full max-w-xs mx-auto lg:mx-0 p-8 rounded-2xl border-4 border-dashed border-red-200 bg-gradient-to-b from-red-50 to-white flex flex-col items-center text-center shadow-inner">
            <p className="text-xs font-bold text-red-800 uppercase tracking-widest">Posigraf</p>
            <p className="text-lg font-black text-slate-900 mt-2">Voucher Natal 2026</p>
            <p className="text-3xl font-black text-red-700 mt-4">R$ {v.valor}</p>
            <div className="mt-6 w-36 h-36 bg-white border-2 border-slate-200 rounded-xl flex items-center justify-center">
              <QrCode className="text-slate-800" size={96} strokeWidth={1} />
            </div>
            <p className="text-[10px] text-slate-500 mt-4">{v.colaborador}</p>
          </div>
        </CardContent>
      </Card>
    </div>
