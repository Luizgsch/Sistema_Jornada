          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#09090b] text-zinc-500 text-xs uppercase tracking-widest">
              <tr>
                ...
                <th className="py-4 px-6 border-b border-[#27272a]">Cargo / Setor</th>
                <th className="py-4 px-6 border-b border-[#27272a]">Gestor</th>
                <th className="py-4 px-6 border-b border-[#27272a]">Contrato</th>
                <th className="py-4 px-6 border-b border-[#27272a]">Admissão</th>
                <th className="py-4 px-6 border-b border-[#27272a]">Status</th>
                <th className="py-4 px-6 border-b border-[#27272a] text-center">Ações</th>
              </tr>
            </thead>
            <tbody>
              {mockOperacoesColaboradores.map((colab) => (
                <tr key={colab.matricula} className="group hover:bg-zinc-800/30 transition-colors">
                  <td className="py-4 px-6 border-b border-[#27272a] font-mono text-xs text-zinc-600">{colab.matricula}</td>
                  ...
                  <td className="py-4 px-6 border-b border-[#27272a] text-zinc-500 text-xs">{colab.gestor}</td>
                  <td className="py-4 px-6 border-b border-[#27272a]">
                    <span className="text-xs font-medium px-2.5 py-0.5 bg-zinc-800 rounded-full border border-zinc-700 text-zinc-400">
                      {colab.contrato}
                    </span>
                  </td>
                  <td className="py-4 px-6 border-b border-[#27272a] text-xs text-zinc-600 font-mono">{colab.admissao}</td>
                  <td className="py-4 px-6 border-b border-[#27272a]">
                  ...
                  <td className="py-4 px-6 border-b border-[#27272a]">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button className="p-1.5 hover:bg-zinc-700 text-zinc-500 rounded-lg transition-colors">
        ...
        {selectedColab && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 w-full max-w-xl bg-[#18181b] border-l border-[#27272a] z-[110] flex flex-col"
            >
              <div className="p-8 border-b border-[#27272a] bg-[#09090b] flex items-start justify-between">
                <div className="flex items-center gap-5">
                  <div className="w-20 h-20 rounded-2xl bg-primary/20 border border-primary/30 flex items-center justify-center text-3xl font-bold text-primary">
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="text-xs bg-zinc-800 px-2 py-0.5 rounded-full border border-[#27272a] text-zinc-500 font-mono">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <Section title="Informações Profissionais">
                  <div className="grid grid-cols-2 gap-4 bg-[#09090b] p-4 rounded-xl border border-[#27272a]">
                ...
                <Section title="Uniforme Registrado">
                  <div className="flex items-center gap-4 p-4 rounded-xl bg-[#09090b] border border-[#27272a]">
                ...
                <Section title="Evolução Corporativa (Treinamentos)">
                  <div className="p-5 border border-[#27272a] rounded-xl bg-[#09090b] relative overflow-hidden group">
              ...
              <div className="p-6 border-t border-[#27272a] bg-[#09090b] flex items-center gap-3">
                <button
                  onClick={() => info("Modo de Edição", `Editando os dados de ${selectedColab.nome}`)}
                  className="flex-1 inline-flex items-center justify-center gap-2 h-11 bg-[#18181b] border border-[#27272a] rounded-xl font-bold hover:bg-zinc-800 transition-colors text-[#e7e5e4] text-sm"
                >
                  <Edit size={16} />
                  Editar Dados
                </button>
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-2 h-11 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 transition-colors text-sm"
                >
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              className="w-full h-10 border border-[#27272a] bg-[#09090b] rounded-lg px-3 text-[#e7e5e4] placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600 text-sm"
...
function StatCard({ title, value, icon: Icon, color }: any) {
  return (
    <div className="bg-[#18181b] rounded-xl border border-[#27272a] hover:border-zinc-600 p-8 flex flex-col gap-4 transition-colors">
      <div className={`p-2 rounded-lg bg-zinc-800 w-fit ${color}`}>
        <Icon size={16} />
      </div>
      <div>
...
function Section({ title, children }: any) {
  return (
    <div className="space-y-4">
      <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-600 border-b border-[#27272a] pb-2">{title}</h3>
      {children}
    </div>
  );
}
